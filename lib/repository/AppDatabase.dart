...
class AppDatabase {
  ...
  Future<List<Task>> getTasksByDay(DateTime date) async {
    ...
    if(r != null) {
      for(var result in r) {
        print("${result[_timeField]} => ${result[_dateField]}");
        tasks.add(Task(
  ...
  Future<List<Task>> getTasksByRange(DateTime from, DateTime to) async {
    var  tasks = <Task> [];
    var r  = await database?.query(_taskTable, where: "$_dateField <= ? AND $_dateField >= ?", whereArgs: [from, to]);
    if(r != null) {
      for(var result in r) {
        tasks.add(Task(
            int.tryParse("${result[_idField]}") ?? -1,
